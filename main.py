...
def update_rows(table):
    ...
    confirm_update = input("Оновити усі рядки? y/n?")
    if confirm_update.lower() == 'y':
        query = update(table).where(getattr(table.c, condition_column) == condition_value).values(new_values)
        conn.execute(query)
        conn.commit()

        print("Рядки успішно оновлені!")
    else:
        print("Оновлення відмінено")

def delete_rows(table):
    columns = table.columns.keys()
    print("Доступні колонки для умови видалення: ")
    for idx, column in enumerate(columns, start=1):
        print(f"{idx}.{column}")
    ...
    confirm_update = input("Видалити усі рядки з цієї таблиці?  y/n?")
    if confirm_update.lower() == 'y':
